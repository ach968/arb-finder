...
def arb_opportunity_blueprint_get():
    ...
    try:
        if get_latest_data:
            max_time_sent = db.session.query(db.func.max(ArbOpportunity.time_sent)).scalar()
            opportunities = ArbOpportunity.query.filter_by(time_sent=max_time_sent).all()
            results = [serialize_arb_opportunity(opportunity) for opportunity in opportunities]
            return results
        else:
